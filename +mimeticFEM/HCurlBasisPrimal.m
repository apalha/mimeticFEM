function [epsilon_1] = HCurlBasisPrimal(xi, eta, p, tensorGrid)
%HCurlBasisPrimal Returns 2 x p x (p+1) 2D vector basis of HCurl primal space
%
%
% Computes the 2D HCurl primal space basis functions by tensor product of 1D
...
%   OUTPUTS
%   -------
%       epsilon_1 :: evaluation of the primal HCurl basis functions at the points
%                    - [xi(k), eta(k)] if tensorGrid = true
%                    - [xi(i), eta(j)] if tensorGrid = false
%                       epsilon(i, j) corresponds to the basis 
%                          i = (i_xi-1)x(p+1) + i_eta if i <= p x (p+1) --> dxi basis
%                          i = (i_xi-1)xp + i_eta if i > p x (p+1) --> deta basis
%                       and node
%                          j
...
%   Revisions:  2020-02-15 (apalha) First implementation.
    ...
    if tensorGrid  % use the nodes as tensor grid
        % Compute the HCurl basis functions based on tensor product of edge basis
        ...
        epsilon_1(1:p*(p+1), :) = kron(mimeticFEM.EdgePoly(xi, p, 'Lobatto'), mimeticFEM.LobattoPoly(eta, p));  % dxi basis
        epsilon_1((p*(p+1)+1):end, :) = kron(mimeticFEM.LobattoPoly(xi, p), mimeticFEM.EdgePoly(eta, p, 'Lobatto'));  % deta basis
        ...
        % dxi basis
        epsilon_1_xi = mimeticFEM.EdgePoly(xi(:), p, 'Lobatto');
        epsilon_1_eta = mimeticFEM.LobattoPoly(eta(:), p);
        ...
        % deta basis
        epsilon_1_xi = mimeticFEM.LobattoPoly(xi(:), p);
        epsilon_1_eta = mimeticFEM.EdgePoly(eta(:), p, 'Lobatto');
        
        for kXi = 1:(p+1)
            for kEta = 1:p
                kBasis = (kXi-1)*p + kEta + p * (p+1);
                epsilon_1(kBasis, :) = epsilon_1_xi(kXi, :) .* epsilon_1_eta(kEta, :);
            end
        end
    end
