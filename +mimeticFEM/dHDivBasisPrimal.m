function D = dHDivBasisPrimal(p)
%dHDivBasisPrimal computes the discrete exterior derivative of elements 
%   of HDivBasisPrimal.
%
%   D = dHDivBasisPrimal(p)
%
%   where D is the matrix of 1 and -1 entries that represents the discrete
%   exterior derivate (xi and eta part) of discrete one forms. It is a
%   sparse matrix.

%   Copyright 2020 Artur Palha
%   $Revision: 1.0 $  $Date: 2020/08/14$

    % xi part
    iiXiMinus = (1:(p*p))';
    jjXiMinus = (1:(p*p))';
    iiXiPlus = iiXiMinus;
    jjXiPlus = jjXiMinus + p;


    % eta part
    iiEtaMinus = iiXiMinus;
    iiEtaPlus = iiEtaMinus;
    jjEtaMinus = zeros([p*p 1]);
    
    for l = 1:p
        for k = 1:p
            jjEtaMinus((l-1)*p + k) = (l-1)*(p+1) + k + p*(p+1);
        end
    end
    
    jjEtaPlus = jjEtaMinus + 1;
    
    % values
    valuesMinus = -ones([p*p 1]);
    valuesPlus = ones([p*p 1]);
    
    D = sparse([iiXiMinus; iiXiPlus; iiEtaMinus; iiEtaPlus], [jjXiMinus; jjXiPlus; jjEtaMinus; jjEtaPlus], [valuesMinus; valuesPlus; valuesMinus; valuesPlus], p^2, 2*p*(p+1));
end