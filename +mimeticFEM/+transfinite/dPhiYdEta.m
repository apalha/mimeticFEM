function dPhiYdEtaEvaluated = dPhiYdEta(element,xi,eta,n,gamma,dgamma)
% dPhiYdEta computes the dPhiYdEta term of the metric for
...
%   USAGE
%   -----
%       dPhiYdEtaEvaluated = dPhiYdEta(element,xi,eta,n,gamma,dgamma)
%
%   INPUTS
%   ------
...
%   OUTPUTS
%   -------
%       dPhiYdEtaEvaluated  :: the evaluation of the eta derivative of the mapping
...
%   Revisions:  2015-08-05 (apalha) First implementation.
    ...
    [~,gamma1y0] = gamma{1}(0.0);
    [~,gamma1y1] = gamma{1}(1.0);
    [~,gamma3y0] = gamma{3}(0.0);
    [~,gamma3y1] = gamma{3}(1.0);
    
    % evaluate the derivatives of the mappings
    %[dgamma1xds,dgamma1yds] = dgamma{1}(s);
    [~,dgamma2ydt] = dgamma{2}(t);
    %[dgamma3xds,dgamma3yds] = dgamma{3}(s);
    [~,dgamma4ydt] = dgamma{4}(t);


    dPhiYdEtaEvaluated = (1.0-s).*dgamma4ydt + s.*dgamma2ydt - gamma1ys +...
                         gamma3ys - (1.0-s).*(-gamma1y0 + gamma3y0) -...
                         s.*(-gamma1y1 + gamma3y1);
    
    dPhiYdEtaEvaluated = dPhiYdEtaEvaluated*deltaT*0.5;
end
