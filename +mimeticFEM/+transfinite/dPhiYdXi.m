function dPhiYdXiEvaluated = dPhiYdXi(element,xi,eta,n,gamma,dgamma)
% dPhiYdXi computes the dPhiYdXi term of the metric for
...
%   USAGE
%   -----
%       dPhiYdXiEvaluated = dPhiYdXi(element,xi,eta,n,gamma,dgamma)
%
%   INPUTS
%   ------
...
%   OUTPUTS
%   -------
%       dPhiYdXiEvaluated  :: the evaluation of the eta derivative of the mapping
...
%   Revisions:  2015-08-05 (apalha) First implementation.
    ...
    [~,dgamma1yds] = dgamma{1}(s);
    %[dgamma2xdt,dgamma2ydt] = dgamma{2}(t);
    [~,dgamma3yds] = dgamma{3}(s);
    %[dgamma4xdt,dgamma4ydt] = dgamma{4}(t);


    dPhiYdXiEvaluated = -gamma4yt + gamma2yt + (1.0-t).*dgamma1yds + t.*dgamma3yds +...
                        ((1.0-t).*gamma1y0 + t.*gamma3y0) - ((1.0-t).*gamma1y1 + t.*gamma3y1);
    
    dPhiYdXiEvaluated = dPhiYdXiEvaluated*deltaT*0.5;
end
