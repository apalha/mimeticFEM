function dPhiXdEtaEvaluated = dPhiXdEta(element,xi,eta,n,gamma,dgamma)
...
%   USAGE
%   -----
%       dPhiXdEtaEvaluated = dPhiXdEta(element,xi,eta,n,gamma,dgamma)
%
%   INPUTS
%   ------
...
%   Revisions:  2015-08-05 (apalha) First implementation.
    ...
    [dgamma2xdt,~] = dgamma{2}(t);
    %[dgamma3xds,dgamma3yds] = dgamma{3}(s);
    [dgamma4xdt,~] = dgamma{4}(t);


    dPhiXdEtaEvaluated = (1.0-s).*dgamma4xdt + s.*dgamma2xdt - gamma1xs + gamma3xs -...
                         (1.0-s).*(-gamma1x0 + gamma3x0) - s.*(-gamma1x1 + gamma3x1);
    
    dPhiXdEtaEvaluated = dPhiXdEtaEvaluated*deltaT*0.5;
end
