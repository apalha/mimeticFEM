function [dgamma1xds_evaluated,dgamma1yds_evaluated] = dgamma1ds(s,epsilon,kappa,delta)
...
%   USAGE
%   -----
%       [dgamma1xds_evaluated,dgamma1yds_evaluated] = dgamma1ds(s,epsilon,kappa,delta)
%
%
%   INPUTS
%   ------
...
%   Revisions:  2015-08-05 (apalha) First implementation.
    ...
    alpha = asin(delta);
    s = (2.0*pi/4.0)*s - 3.0*pi/4.0;
    
    dgamma1xds_evaluated = -epsilon.*(1.0+alpha*cos(s)).*sin(s+alpha.*sin(s)).*(2.0*pi/4.0);
    dgamma1yds_evaluated = epsilon*kappa*cos(s)*2.0*pi/4.0;
