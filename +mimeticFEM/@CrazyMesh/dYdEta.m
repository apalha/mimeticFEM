function dYdEtaEvaluated = dYdEta(obj,element,xi,eta)
% dYdEta computes the dXdEta term of the metric for
% at the local positions (xi,eta) for the element. For the crazy grid.
%
%   USAGE
%   -----
%       dYdEtaEvaluated = obj.dYdEta(element,xi,eta)
%
%   INPUTS
%   ------
...
%                          the same time, speeding up the computation. (apalha)
    ...
    dYdEtaEvaluated = 0.5*deltaY*(obj.yBounds(2)-obj.yBounds(1))*0.5 +...
                         pi*deltaY*0.5*obj.cc*...
                         cos(bsxfun(@plus,pi*(eta+1)*0.5*deltaY,pi*yLeft)).*...
                         sin(bsxfun(@plus,pi*(xi+1)*0.5*deltaX,pi*xLeft))*...
                         (obj.yBounds(2)-obj.yBounds(1))*0.5;
end