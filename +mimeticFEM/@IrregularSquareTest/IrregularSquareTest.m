classdef IrregularSquareTest %< mimeticFEM2.Mesh
    ...
    methods
        function obj = IrregularSquareTest(n, randomizeNodes, element)
            obj.numElements_base = n*n;
            obj.numElements = 1;
            obj.n_base = [n, n];
            obj.n = [1, 1];
            obj.dim = [2, 2];
        ...
        function nodesOfElements = computeNodesOfElements(obj, randomizeNodes)
            ...
            [nodesCoordinatesX, nodesCoordinatesY] = meshgrid(linspace(-1, 1, obj.n_base(1)+1), linspace(-1, 1, obj.n_base(2)+1));

            if randomizeNodes > 0
                %dx_random = 0.0;%randomizeNodes;
                %dy_random = randomizeNodes;
                %nodesCoordinatesX(2:end-1, 2:end-1) = nodesCoordinatesX(2:end-1, 2:end-1) + dx_random*(2*rand(size(nodesCoordinatesX(2:end-1, 2:end-1))) - 1.0) * 2.0/(obj.n_base(1));
                %nodesCoordinatesY(2:end-1, 2:end-1) = nodesCoordinatesY(2:end-1, 2:end-1) + dy_random*(2*rand(size(nodesCoordinatesY(2:end-1, 2:end-1))) - 1.0) * 2.0/(obj.n_base(2));
                iDeformed = 2:2:obj.n_base(1);
                jDeformed = 2:2:obj.n_base(2);
                nodesCoordinatesY(iDeformed, jDeformed) = nodesCoordinatesY(iDeformed, jDeformed) + randomizeNodes * 2.0/(obj.n_base(2));
                jDeformed = 1:2:obj.n_base(1);
                nodesCoordinatesY(iDeformed, jDeformed) = nodesCoordinatesY(iDeformed, jDeformed) - randomizeNodes * 2.0/(obj.n_base(2));
